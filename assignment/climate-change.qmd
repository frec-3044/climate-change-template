...
To read the code, you will use a new function called `read_table`.  It is similar to `read_csv` except that it is looking for spaces between column entries rather than commas (remember that csv stands for comma-separated values).  Others include `read_tsv` which uses tabs to separate entries.  You can discover the separation type by putting <https://gml.noaa.gov/webdata/ccgg/trends/co2/co2_mm_mlo.txt> into your webbrowser and examining the file.  The code also includes an argument for comment, which denotes the character used define a line in the file as being a commment rather than data or a header.  Looking at the file also reveals that the column headers on lines with comments character so they won't be read in.  You will use the argument `col_names` to define the headers manually.  Finally, `NA` values are defined by the numbers -1 and -99 so that needs to be defined using the `na` argument (otherwise they will be read in as numeric values).
...
- Describe class of each column and what units it is measured in. 
- What is the different between "global temperature" and "global temperature anomaly"? 
...
You will need to figure out the delimiter for the file and any comments or skips.  You will also need to be sure that you have column names.  You will not directly use the code above for reading in the CO2 data, but that code provides helpful tips for reading in data.
...
- Plot data.  
- Combine this time series with the Mauna Loa data (the Warm-up exercise in this assignment).
